...
import { BinarySmlFile, BinarySmlStreamReader, BinarySmlStreamWriter, SmlStreamReader, SmlStreamWriter, SyncBinarySmlStreamReader, SyncBinarySmlStreamWriter, SyncSmlStreamReader, SyncSmlStreamWriter } from "@stenway/sml-io";
import { TblDocument } from "@stenway/tbl";
// ----------------------------------------------------------------------
export class TblFile {
    static saveSync(document, filePath, aligned = false, whitespaceBetween = null, rightAligned = null, overwriteExisting = true) {
    ...
    static async save(document, filePath, aligned = false, whitespaceBetween = null, rightAligned = null, overwriteExisting = true) {
        const content = document.toString(aligned, whitespaceBetween, rightAligned);
        await ReliableTxtFile.writeAllText(content, filePath, document.encoding, overwriteExisting);
    }
    static saveMinifiedSync(document, filePath, overwriteExisting = true) {
        const content = document.toMinifiedString();
        ReliableTxtFile.writeAllTextSync(content, filePath, document.encoding, overwriteExisting);
    }
    static async saveMinified(document, filePath, overwriteExisting = true) {
        const content = document.toMinifiedString();
        await ReliableTxtFile.writeAllText(content, filePath, document.encoding, overwriteExisting);
    }
