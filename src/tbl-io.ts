...
import { BinarySmlFile, BinarySmlFileHandle, BinarySmlStreamReader, BinarySmlStreamWriter, SmlStreamReader, SmlStreamWriter, SyncBinarySmlFileHandle, SyncBinarySmlStreamReader, SyncBinarySmlStreamWriter, SyncSmlStreamReader, SyncSmlStreamWriter } from "@stenway/sml-io"
import { TblDocument } from "@stenway/tbl"

// ----------------------------------------------------------------------

export abstract class TblFile {
	static saveSync(document: TblDocument, filePath: string, aligned: boolean = false, whitespaceBetween: string | null = null, rightAligned: boolean[] | null = null, overwriteExisting: boolean = true) {
		const content: string = document.toString(aligned, whitespaceBetween, rightAligned)
		ReliableTxtFile.writeAllTextSync(content, filePath, document.encoding, overwriteExisting)
	}

	static async save(document: TblDocument, filePath: string, aligned: boolean = false, whitespaceBetween: string | null = null, rightAligned: boolean[] | null = null, overwriteExisting: boolean = true) {
		const content: string = document.toString(aligned, whitespaceBetween, rightAligned)
		await ReliableTxtFile.writeAllText(content, filePath, document.encoding, overwriteExisting)
	}

	static saveMinifiedSync(document: TblDocument, filePath: string, overwriteExisting: boolean = true) {
		const content: string = document.toMinifiedString()
		ReliableTxtFile.writeAllTextSync(content, filePath, document.encoding, overwriteExisting)
	}

	static async saveMinified(document: TblDocument, filePath: string, overwriteExisting: boolean = true) {
		const content: string = document.toMinifiedString()
		await ReliableTxtFile.writeAllText(content, filePath, document.encoding, overwriteExisting)
	}
...
export abstract class BinaryTblFile {
	static saveSync(document: TblDocument, filePath: string, overwriteExisting: boolean = true) {
		const smlDocument = new SmlDocument(document.toElement())
		BinarySmlFile.saveSync(smlDocument, filePath, overwriteExisting)
	}

	static async save(document: TblDocument, filePath: string, overwriteExisting: boolean = true) {
		const smlDocument = new SmlDocument(document.toElement())
		await BinarySmlFile.save(smlDocument, filePath, overwriteExisting)
	}

	static loadSync(filePath: string): TblDocument {
		const smlDocument = BinarySmlFile.loadSync(filePath)
		return TblDocument.parseElement(smlDocument.root)
	}

	static async load(filePath: string): Promise<TblDocument> {
		const smlDocument = await BinarySmlFile.load(filePath)
		return TblDocument.parseElement(smlDocument.root)
	}

	static appendRowsSync(rows: (string | null)[][], templateDocument: TblDocument, filePath: string) {
		if (rows.length === 0) { return }
		const writer = SyncBinaryTblStreamWriter.create(templateDocument, filePath, WriterMode.CreateOrAppend)
		try {
			writer.writeRows(rows)
		} finally {
			writer.close()
	...
	static async appendRows(rows: (string | null)[][], templateDocument: TblDocument, filePath: string) {
		if (rows.length === 0) { return }
		const writer = await BinaryTblStreamWriter.create(templateDocument, filePath, WriterMode.CreateOrAppend)
		try {
			await writer.writeRows(rows)
		} finally {
			await writer.close()
...
export class SyncBinaryTblStreamReader {
	private reader: SyncBinarySmlStreamReader
	readonly header: TblDocument
	...
	private constructor(reader: SyncBinarySmlStreamReader, header: TblDocument) {
		this.reader = reader
		this.header = header
	}

	static create(filePath: string, chunkSize: number = 4096): SyncBinaryTblStreamReader {
		const reader = SyncBinarySmlStreamReader.create(filePath, chunkSize)
		try {
			return this.internalCreate(reader)
		} catch (error) {
			reader.close()
	...
	private static internalCreate(reader: SyncBinarySmlStreamReader): SyncBinaryTblStreamReader {
		if (!reader.root.hasName("Table")) { throw new Error("Not a valid table document") }
		...
		const columnNames: string[] = [columnNamesAttribute.name, ...(columnNamesAttribute.values as string[])]
		const header = new TblDocument(columnNames)
		if (metaElement !== null) { header.meta.parse(metaElement) }
		return new SyncBinaryTblStreamReader(reader, header)
	}
...
export class BinaryTblStreamReader {
	private reader: BinarySmlStreamReader
	readonly header: TblDocument
	...
	private constructor(reader: BinarySmlStreamReader, header: TblDocument) {
		this.reader = reader
		this.header = header
	}

	static async create(filePath: string, chunkSize: number = 4096): Promise<BinaryTblStreamReader> {
		const reader = await BinarySmlStreamReader.create(filePath, chunkSize)
		try {
			return await this.internalCreate(reader)
		} catch (error) {
			await reader.close()
	...
	static async getAppendReader(writer: BinarySmlStreamWriter): Promise<BinaryTblStreamReader> {
		if (!writer.existing) { throw new Error(`Writer is not in append mode`) }
		const reader = await BinarySmlStreamReader.getAppendReader(writer)
		return await this.internalCreate(reader)
	}

	private static async internalCreate(reader: BinarySmlStreamReader): Promise<BinaryTblStreamReader> {
		if (!reader.root.hasName("Table")) { throw new Error("Not a valid table document") }
		...
		const header = new TblDocument(columnNames)
		if (metaElement !== null) { header.meta.parse(metaElement) }
		return new BinaryTblStreamReader(reader, header)
	}
...
export class SyncBinaryTblStreamWriter {
	private writer: SyncBinarySmlStreamWriter
	readonly header: TblDocument
	...
	private constructor(writer: SyncBinarySmlStreamWriter, header: TblDocument) {
		this.writer = writer
		this.header = header
	}

	static create(templateDocument: TblDocument, filePath: string, mode: WriterMode = WriterMode.CreateOrOverwrite): SyncBinaryTblStreamWriter {
		const writer = SyncBinarySmlStreamWriter.create("Table", filePath, mode)
		try {
			let header: TblDocument
			if (writer.existing) {
				const reader = SyncBinaryTblStreamReader.getAppendReader(writer)
				if (reader.header.columnCount !== templateDocument.columnCount) { throw new Error(`Column count mismatch`) }
				header = reader.header
			} else {
				if (templateDocument.meta.hasAny) {
			...
			return new SyncBinaryTblStreamWriter(writer, header)
		} catch (error) {
			writer.close()
			throw error
		}
...
export class BinaryTblStreamWriter {
	private writer: BinarySmlStreamWriter
	readonly header: TblDocument
	...
	private constructor(writer: BinarySmlStreamWriter, header: TblDocument) {
		this.writer = writer
		this.header = header
	}

	static async create(templateDocument: TblDocument, filePath: string, mode: WriterMode = WriterMode.CreateOrOverwrite): Promise<BinaryTblStreamWriter> {
		const writer = await BinarySmlStreamWriter.create("Table", filePath, mode)
		try {
			let header: TblDocument
			if (writer.existing) {
				const reader = await BinaryTblStreamReader.getAppendReader(writer)
				if (reader.header.columnCount !== templateDocument.columnCount) { throw new Error(`Column count mismatch`) }
				header = reader.header
			} else {
				if (templateDocument.meta.hasAny) {
			...
			return new BinaryTblStreamWriter(writer, header)
		} catch (error) {
			await writer.close()
			throw error
		}
